            ...
            <div class="width_100 pos_re_block">
                <div class="cantent_block ">
                                ...
                                <table class="table  table-hover table_style table_list1 " id="aa" style="border-left: 1px solid #ccc; border-right: 1px solid #ccc!important;">
                                    ...
                                    <c:forEach items="${detailinfo}" var="item" varStatus="st">
                                        <tr class="${st.index%2==1 ? '':'odd' }">
                                            ...
                                            <td align="center" title="${item.friendbz}"><div style="width:130px;white-space: nowrap;text-overflow:ellipsis; overflow:hidden;">${item.friendbz}</div></td>
                                            <td align="center">${item.friendszd}</td>
                                            <td align="center" title="${item.friendqm}"><div style="width:80px;white-space: nowrap;text-overflow:ellipsis; overflow:hidden;">${item.friendqm}</div></td>
                                        </tr>
                                    </c:forEach>
                                    <c:choose>
                            ...
                            <c:choose>
                                <c:when test="${detailinfo!=null && detailinfo.size()!=0}">
                                    <div  class="page_nmber">
                                        ...
                                        <a href="${pageContext.request.contextPath}/phonewxFriend/seach?pageNo=${page.topPageNo }"><input type="button" name="fristPage" value="首页" /></a>
                                        <c:choose>
                                            <c:when test="${page.pageNo!=1}">
                                                <a href="${pageContext.request.contextPath}/phonewxFriend/seach?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <c:choose>
                                            <c:when test="${page.pageNo != page.totalPages}">
                                                <a href="${pageContext.request.contextPath}/phonewxFriend/seach?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                                <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="${pageContext.request.contextPath}/phonewxFriend/seach?pageNo=${page.bottomPageNo }"><input type="button" name="lastPage" value="尾页" /></a>
                    ...
                    <div class="sidebar_right pos_re">
                        ...
                        <div class=" ">
                            ...
                            <div>
                                <form action="${pageContext.request.contextPath}/phonewxFriend/seachCode" method="post">
                                    <div class="form-group_search  fl_l width100" >
                                        <span style="margin-left: 10px;color: #444;padding-bottom: 10px;">查询方式</span>
                                        <select name="seachCondition" class="width100" STYLE="margin-bottom: 20px;">
                                            <option value="wechatno"<c:if test="${wxFriendxxSeachCondition=='wechatno'}">selected="selected"</c:if>>微信账号</option>
                                            <option value="name" <c:if test="${wxFriendxxSeachCondition=='name'}">selected="selected"</c:if>>姓名</option>
                                            <option value="fdwechatno" <c:if test="${wxFriendxxSeachCondition=='fdwechatno'}">selected="selected"</c:if>>好友账号</option>
                                            <option value="friendnc" <c:if test="${wxFriendxxSeachCondition=='friendnc'}">selected="selected"</c:if>>好友昵称</option>
                                            <option value="friendbz" <c:if test="${wxFriendxxSeachCondition=='friendbz'}">selected="selected"</c:if>>好友备注</option>
                                            <option value="sjhm" <c:if test="${wxFriendxxSeachCondition=='sjhm'}">selected="selected"</c:if> >手机号码</option>
                                        </select>
                                        <%--<input  style="margin-left: 10px;" type="checkbox" name="usable" value="1" <c:if test="${usable eq '1'}">checked="checked"</c:if>>上次条件有效--%>
                                        <textarea  class="form-control02 seachCode fl_l width100" id="seachCode" placeholder="请输入要查询内容" name="seachCode" >${wxFriendxxSeachCode}</textarea>
                                    </div>
                ...
                <button class="sideBar_r_button" data-toggle="modal"
                        data-target="#myModal">文件夹导入</button>
                           <button  type="button"  class="sideBar_r_button"  onclick="location.href='${pageContext.request.contextPath}/wxFriend/download'" >数据导出</button>
                       </span>
                                    </div>
                                </div>
                            </div>
