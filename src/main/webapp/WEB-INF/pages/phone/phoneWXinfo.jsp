                ...
                <div class="cantent_block ">
                            ...
                            <c:choose>
                                <c:when test="${detailinfo!=null && detailinfo.size()!=0}">
                                    <div  class="page_nmber">
                                        ...
                                        <a href="${pageContext.request.contextPath}/phonewxPhone/seach?pageNo=${page.topPageNo }"><input type="button" name="fristPage" value="首页" /></a>
                                        <c:choose>
                                            <c:when test="${page.pageNo!=1}">
                                                <a href="${pageContext.request.contextPath}/phonewxPhone/seach?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <c:choose>
                                            <c:when test="${page.pageNo != page.totalPages}">
                                                <a href="${pageContext.request.contextPath}/phonewxPhone/seach?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                                <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="${pageContext.request.contextPath}/phonewxPhone/seach?pageNo=${page.bottomPageNo }"><input type="button" name="lastPage" value="尾页" /></a>
                    ...
                    <div class="sidebar_right pos_re">
                        ...
                        <div class=" ">
                            ...
                            <div>
                                <form action="${pageContext.request.contextPath}/phonewxPhone/seachCode" method="post">
                                    <div class="form-group_search  fl_l width100" >
                                        <span style="margin-left: 10px;color: #444;padding-bottom: 10px;">查询方式</span>
                                        <select name="seachCondition" class="width100" STYLE="margin-bottom: 20px;">
                                            <option value="wechatno"<c:if test="${wxZhxxSeachCondition=='wechatno'}">selected="selected"</c:if>>微信号</option>
                                            <option value="qq"<c:if test="${wxZhxxSeachCondition=='qq'}">selected="selected"</c:if>>QQ号</option>
                                            <option value="name" <c:if test="${wxZhxxSeachCondition=='name'}">selected="selected"</c:if>>姓名</option>
                                            <option value="sjhm" <c:if test="${wxZhxxSeachCondition=='sjhm'}">selected="selected"</c:if>>手机号码</option>
