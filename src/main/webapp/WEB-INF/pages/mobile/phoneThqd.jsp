                ...
                <div class="cantent_block ">
                            ...
                            <div >
                                ...
                                <table class="table  table-hover table_style table_list1 " id="aa" style="border-left: 1px solid #ccc; border-right: 1px solid #ccc!important;">
                                    <tr>
                                        <td colspan="9"  align="center" class="dropdown_index" style="background-color: #eee;">
                                            <div class="dropdown " style="color: #333">
                                                <strong>通话清单(${aj.caseName})</strong>
                                    ...
                                    <tr align="center">
                                        ...
                                        <td width="8%">对方姓名</td>
                                        <td width="12%">对方号码</td>
                                        <td width="10%"><a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=1&orderby=callDate">呼叫日期</a></td>
                                        <td width="10%"><a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=1&orderby=callTime">呼叫时间</a></td>
                                        <td width="10%"><a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=1&orderby=callValue">呼叫时长</a></td>
                                        <td width="10%"><a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=1&orderby=callType">呼叫类型</a></td>
                                    </tr>
                                    ...
                                    <c:forEach items="${detailinfo}" var="item" varStatus="st">
                                        <tr class="${st.index%2==1 ? '':'odd' }">
                                            ...
                                            <td align="center">${item.callDate}</td>
                                            <td align="center">${item.callTime}</td>
                                            <td align="center">${item.callValue}</td>
                                            <td align="center">${item.callType}</td>
                                        </tr>
                                    </c:forEach>
                                    <c:choose>
                            ...
                            <c:choose>
                                <c:when test="${detailinfo!=null && detailinfo.size()!=0}">
                                    <div  class="page_nmber">
                                        ...
                                        <a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=${page.topPageNo }"><input type="button" name="fristPage" value="首页" /></a>
                                        <c:choose>
                                            <c:when test="${page.pageNo!=1}">
                                                <a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <c:choose>
                                            <c:when test="${page.pageNo != page.totalPages}">
                                                <a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                                <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="${pageContext.request.contextPath}/phoneThqd/seach?pageNo=${page.bottomPageNo }"><input type="button" name="lastPage" value="尾页" /></a>
                    ...
                    <div class="sidebar_right pos_re">
                        ...
                        <div class=" ">
                            ...
                            <div>
                                <form action="${pageContext.request.contextPath}/phoneThqd/seachCode" method="post">
                                    <div class="form-group_search  fl_l width100" >
                                        <span style="margin-left: 10px;color: #444;padding-bottom: 10px;">查询方式</span>
                                        <select name="seachCondition" class="width100" STYLE="margin-bottom: 20px;">
                                            <option value="uName" <c:if test="${phoneThqdSeachCondition=='uName'}">selected="selected"</c:if>>用户姓名</option>
                                            <option value="uNumber"<c:if test="${phoneThqdSeachCondition=='uNumber'}">selected="selected"</c:if>>用户号码</option>
                                            <option value="pName"<c:if test="${phoneThqdSeachCondition=='pName'}">selected="selected"</c:if>>对方姓名</option>
                                            <option value="pNumber" <c:if test="${phoneThqdSeachCondition=='pNumber'}">selected="selected"</c:if>>对方号码</option>
                                        </select>
                                        <%--<input  style="margin-left: 10px;" type="checkbox" name="usable" value="1" <c:if test="${usable eq '1'}">checked="checked"</c:if>>上次条件有效--%>
                                        <textarea  class="form-control02 seachCode fl_l width100" id="seachCode" placeholder="请输入要查询内容,加%模糊查询 例如:%xxx%" name="seachCode" >${phoneThqdSeachCode}</textarea>
                                    </div>
