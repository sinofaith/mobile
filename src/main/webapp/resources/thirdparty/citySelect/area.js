		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				...
				for(i=0;i<ar.length;i++){
					if(document.getElementById(sv[v-1]).value === ar[i]){
						changeLength = length
					}
					options[length]=new Option(ar[i],ar[i]);
				}//end for
				if(v){ options[changeLength].selected = true; }
			}
		}//end if v
...
dsy.add("0_22_19",["康定县","炉城镇","泸定县","泸桥镇","丹巴县","章谷镇","九龙县","呷尔镇","雅江县","河口镇","道孚县","鲜水镇","炉霍县","新都镇","甘孜/县","甘孜镇","新龙县","茹龙镇","德格县","更庆镇","白玉县","建设镇","石渠县","尼呷镇","色达县","色柯镇","理塘县","高城镇","巴塘县","夏邛镇","乡城县","桑披镇","稻城县","金珠镇","得荣县","松麦镇"]);
...
var sv=["s_province_value","s_city_value","s_county_value"];//三个select的name
var opt0 = ["省份","地级市","市、县级市"];//初始值
function _init_area(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	}
	if(document.getElementById("s_province_value").value ===""){
		for(i=0;i<s.length-1;i++){
			change(i+1);
		}
	} else {
