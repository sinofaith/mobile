...
function change(v){
	...
	if(document.getElementById(sv[0]).value === ""){
		str="0";
		for(i=0;i<v;i++){
		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				...
				for(i=0;i<ar.length;i++){
					if(document.getElementById(sv[v]).value ===ar[i] ){
						str=str+"_"+i;
					}
			...
			var strValue =document.getElementById(sv[v]).value;

			$("#"+s[v]).find("option[value='"+strValue+"']").attr("selected",true);
		}//end if v
		if(++v<s.length){change(v);}
	}//End with
	document.getElementById(sv[0]).value ="";
}
...
function selectOptions(){
	...
	for(j=0;i<3;j++){
		length = 0;
		var selectOption = 0;
		var options=new Option(opt0[j],opt0[j]);
		if(dsy.Exists(str)){
			ar = dsy.Items[str];
			for(i=0;i<ar.length;i++){
				if(document.getElementById(sv[j]).value === ar[i]){
					selectOption = i;
					str = str+"_"+i;
				}
			}
			if(j){ options[selectOption].attr("selected",true); }
		}
...
var sv=["s_province_value","s_city_value","s_county_value"];//三个select的name_value
var opt0 = ["省份","地级市","市、县级市"];//初始值
function _init_area(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	}
	if(document.getElementById(sv[0]).value.length !=0){
		change(0);
		//selectOptions();
	}else {
		change(0);
